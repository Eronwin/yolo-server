...
from typing import Optional, List, Tuple, Union, Any, overload, AsyncGenerator, Callable

from sqlalchemy import select, func
from sqlalchemy.exc import IntegrityError, OperationalError
from sqlalchemy.orm.exc import FlushError
from sqlmodel import SQLModel, col, and_, asc, desc
from sqlmodel.ext.asyncio.session import AsyncSession

from backend.schemas.common import PaginatedList, Pagination
from backend.server.bus import EventType, event_bus, Event
...
class ActiveRecordMixin:
    ...
    def primary_key(self):
        ...
        return self.__mapper_.primary_key_form_instance(self)
    ...
    @classmethod
    async def first_by_field(cls, session: AsyncSession, field: dict):
        """
    ...
    async def paginated_by_query(
        ...
        Args:
            ...
            fuzzy_fields (Optional[dict]): Fuzzy match filters as key-value pairs.
            page (int): Page number for pagination, staring from 1. Defaults to 1.
            per_page (int): Number of items per page. Defaults to 100.
            order_by (Optional[List[Tuple[str, str]]]): Sorting criteria as a list of tuples,
                echo containing a field name and sort direction ("asc" or "desc").
        ...
        if fuzzy_fields:
            ...
            statement = statement.where(and_(*fuzzy_conditions))
        ...
        if fuzzy_fields:
            ...
            count_statement = count_statement.where(and_(*fuzzy_conditions))
    ...
    async def create(
        ...
        obj = cls.convert_without_saving(source, update=update)
        if obj is None:
            return None
    ...
    async def create_or_update(
        ...
        if obj is None:
            return None
        pk = cls.__mapper__.primary_key_form_instance(obj)
        if pk[0] is not None:
    ...
    async def update(
        ...
        # Update the object with source and save to the database.
        ...
        await self._pushlish_event(EventType.UPDATED, self)
    ...
    async def delete(self, session: AsyncSession):
        ...
        if self._has_cascade_delete():
            if hasattr(self, "delete_at"):
                ...
                self.delete_at = datetime.now(timezone.utc).replace(tzinfo=None)
                await self.save(session)
    ...
    @classmethod
    async def subscribe(
        cls, session_or_engine: Union[AsyncSession, AsyncSession]
    ) -> AsyncGenerator[Event, None]:
    ...
    def _convert_to_public_class(cls, data: Any) -> Any:
        ...
        class_module = importlib.import_module(cls.__package__)
        public_class = getattr(class_module, f"{cls.__name__}Public", None)
        return public_class.model_value(data) if public_class else data
