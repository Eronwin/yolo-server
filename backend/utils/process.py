...
def add_signal_handlers_in_loop():
    ...
    for sig in (signal.SIGINT, signal.SIGTERM):
        logging.debug(f"Adding signal handler for {sig}")
...
def terminate_processes(processes):
    """
    Terminate a list of processes, attempting graceful termination first,
...
def terminate_process(process):
    """
    Terminate a single process, attempting graceful termination first,
